...
const Footer = () => {
  ...
  const footerLinks: Array<{
    ...
    },
    {
      title: "Network Nodes",
      links: [
        { name: "LinkedIn", href: "https://linkedin.com", external: true, icon: <Linkedin size={14} className="sm:w-4 sm:h-4" /> },
        { name: "Twitter", href: "https://twitter.com", external: true, icon: <Twitter size={14} className="sm:w-4 sm:h-4" /> },
        { name: "GitHub", href: "https://github.com", external: true, icon: <Github size={14} className="sm:w-4 sm:h-4" /> },
      ]
    }
  ];

  return (
    <footer className="relative w-full bg-[#020202] border-t border-white/5 pt-12 sm:pt-16 lg:pt-20 pb-8 sm:pb-12 z-30">
      <div className="container mx-auto">
                    ...
                    <li key={link.name}>
                      {isExternal ? (
                        <a 
                          ...
                          className="text-xs sm:text-sm text-muted-foreground hover:text-white transition-colors flex items-center gap-1.5 sm:gap-2 group"
                        >
        ...
        <div className="pt-6 sm:pt-8 border-t border-white/5 flex flex-col sm:flex-row justify-between items-center gap-4 sm:gap-6">
          <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-6">
            <p className="text-[10px] sm:text-xs text-muted-foreground font-mono">
              © {currentYear} EASYIO TECHNOLOGIES
            </p>
            <div className="hidden sm:block w-1 h-1 rounded-full bg-white/20" />
            <Link href="/contact" className="text-[10px] sm:text-xs text-muted-foreground hover:text-white transition-colors font-mono">
              Security Manifest
            </Link>
          </div>
