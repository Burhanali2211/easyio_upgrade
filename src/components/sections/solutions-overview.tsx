...
const SolutionCard = memo(({ solution, index, Icon, colors }: SolutionCardProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 rounded-2xl sm:rounded-[2.5rem] border border-white/5 bg-white/[0.01] group-hover:border-white/25 group-hover:shadow-[0_0_30px_rgba(255,255,255,0.08)] transition-all duration-500 ease-out" />
      ...
      <div className="relative p-6 sm:p-8 lg:p-12 flex flex-col h-full z-10">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 sm:gap-0 mb-6 sm:mb-12">
          <div className="w-14 h-14 sm:w-20 sm:h-20 rounded-xl sm:rounded-[2rem] bg-white/5 border border-white/10 flex items-center justify-center group-hover:scale-105 transition-transform duration-300">
            <Icon className={`w-6 h-6 sm:w-8 sm:h-8 ${colors.icon}`} />
          </div>
          <span className="text-[9px] sm:text-[10px] font-mono font-bold text-white/30 uppercase tracking-[0.2em] sm:tracking-[0.3em] bg-white/[0.02] px-3 sm:px-4 py-1.5 sm:py-2 rounded-full border border-white/5 w-fit">
            {solution.category}
          </span>
        </div>
        
        <h3 className="text-xl sm:text-2xl lg:text-3xl font-display font-bold text-white mb-3 sm:mb-6">
          {solution.title}
        </h3>
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3 mb-6 sm:mb-12">
          {(solution.features || []).map((feature) => (
            <div key={feature} className="flex items-center gap-1.5 sm:gap-2 py-1.5 sm:py-2 px-2.5 sm:px-3 rounded-lg sm:rounded-xl bg-white/[0.02] border border-white/5 whitespace-nowrap">
              <Zap className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-primary flex-shrink-0" />
              <span className="text-[8px] sm:text-[9px] font-bold text-white/70 uppercase tracking-tight">{feature}</span>
            </div>
          ))}
        </div>
        ...
        <button 
          ...
          className={`flex items-center gap-2 sm:gap-3 text-white font-bold uppercase tracking-[0.15em] sm:tracking-[0.2em] text-[10px] sm:text-[11px] group/btn group-hover:${colors.icon} transition-colors cursor-pointer`}
          aria-label={`Initialize System - ${solution.title}`}
        >
          Initialize System 
          <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-white/5 flex items-center justify-center group-hover:bg-white/10 group-hover:scale-110 transition-all duration-300`}>
...
const SolutionsOverview = memo(({ solutions = [] }: SolutionsProps) => {
  ...
  return (
    <section className="py-16 sm:py-24 lg:py-32 bg-[#020202] relative overflow-hidden z-10" data-section="solutions">
      <div className="container relative z-10">
        <div className="flex flex-col mb-12 sm:mb-16 lg:mb-24">
          ...
          <h2 className="text-[clamp(2rem,10vw,3.5rem)] sm:text-5xl lg:text-8xl font-display font-bold text-white tracking-tighter max-w-4xl animate-fadeIn">
            Universal <span className="text-white/20">Business</span> <br className="hidden sm:block" />
